...
%% 主程序
% 识别低速区域
regions505 = find_low_speed_regions(new_trajectory505,20, 50);
regions669 = find_low_speed_regions(new_trajectory669,20,50);
% 显示低速区域信息
disp('Trajectory505低速区域:');
disp(get_time_space_ranges(new_trajectory505, regions505));
disp('Trajectory669低速区域:');
disp(get_time_space_ranges(new_trajectory669, regions669));
% 执行分阶段DTW匹配
[global_path_sorted, global_path, tau, d,tA,pA,tB,pB] = match_trajectories(new_trajectory505,new_trajectory669,11,50);
% 可视化结果
figure;
plot(tau, d, 'b.');
xlabel('时间滞后 (tau)');
...
% 绘制轨迹一和轨迹二的位置-时间曲线
figure;
plot(tA, pA, 'b-', 'DisplayName', 'Trajectory A');
hold on;
plot(tB, pB, 'g-', 'DisplayName', 'Trajectory B');

% 设置时间轴和位置轴的范围
time_min = min([min(tA), min(tB)]);
time_max = max([max(tA), max(tB)]);
position_min = min([min(pA), min(pB)]);
position_max = max([max(pA), max(pB)]);
xlim([time_min, time_max]);
ylim([position_min, position_max]);
% 
% 绘制匹配点对的虚线连接，每隔10个点绘制一个匹配对
for k = 1:10:size(global_path_sorted,1)
    i = global_path_sorted(k,1);
    j = global_path_sorted(k,2);
    plot([tA(i), tB(j)], [pA(i), pB(j)], 'r--', 'LineWidth', 0.5);
end
%% 匹配对信息
% 提取匹配对的完整信息
...
fixed_points = vehiclesInRangeAt50m1; % 固定检测点坐标  
    ...
    if best_match_index ~= -1
        fixed_point_matches{k} = matched_pairs_info(best_match_index, :);
         if fixed_point_matches{k}.Time_A ~= fixed_point_matches{k}.Time_B
            ...
            % 存储计算出的比值
            fixed_point_ratios4 = [fixed_point_ratios4; ratio_tau_A, ratio_d_A];
        else
            % 获取当前最佳匹配对的时间差和位置差
            tau_A = fixed_point_matches{k}.Time_A - fixed_time;
            d_A = fixed_point_matches{k}.Position_A - fixed_position;
...
fixed_points = vehiclesInRangeAt50m1;
...
fixed_point_ratios(1, :) = [0, 0];  % 第一个固定点比值
fixed_point_ratios(end, :) = [1, 1];  % 最后一个固定点比值
...
%% 669
all_full_trajectories = {};  % 存储所有固定点的调整轨迹
...
figure;
% 遍历所有固定点
for k = 1:size(filtered_fixed_points, 1)
    % 获取当前固定点的时间和位置
    fixed_time = table2array(filtered_fixed_points(k, 2));
    fixed_position = table2array(filtered_fixed_points(k, 3));
    ...
    % 存储当前固定点的调整轨迹
    all_full_trajectories{k} = adjusted_trajectory_A;
...
for i = 1:num_fixed_points
    % 绘制固定点
    fixed_point_time = filtered_fixed_points.Time(i);
    fixed_point_position = filtered_fixed_points.Distance(i);
...
% 输出调整后的轨迹 A
disp('调整后的轨迹 A：');
disp(all_full_trajectories);

%%
figure
hold on;  
% 遍历所有轨迹  
plot(tA, pA, 'g-','LineWidth', 3, 'DisplayName', 'Trajectory A');
hold on;
plot(tB, pB, 'g-', 'LineWidth', 3,'DisplayName', 'Trajectory B');
...
% 
% 绘制匹配点对的虚线连接，每隔10个点绘制一个匹配对
for k = 1:25:size(global_path_sorted,1)
    i = global_path_sorted(k,1);
    j = global_path_sorted(k,2);
    plot([tA(i), tB(j)], [pA(i), pB(j)], 'r--', 'LineWidth', 1.5);
end
% % % % % % % %遍历所有轨迹
for i = 1:size(trajectory_cell1_ngsim,1)
    % 提取当前轨迹的坐标
    trajectory = trajectory_cell1_ngsim{i};
    %  % 查找距离首次达到或超过950m的点
    distances = trajectory.y; % save('trajectories236-239','selected_trajectories','fused_trajectories','all_trajectories2','all_trajectories')
    tolerance = 1; % 允许的误差范围
    threshold_index = find(distances >= 595-tolerance & distances <= 600+tolerance, 1, 'first');
    % 绘制当前灰色轨迹的部分
    % plot(trajectory(:, 1), trajectory(:, 2));
    % % 如果存在这样的点，则在该点位置绘制一个洋红色的点
    if ~isempty(threshold_index)
        plot(trajectory.globaltime(threshold_index), trajectory.y(threshold_index), 'm*', 'MarkerSize', 5);
    end
    threshold_index2 = find(distances >= 30-tolerance & distances <= 35+tolerance, 1, 'first');
    % 绘制当前灰色轨迹的部分
    % plot(trajectory(:, 1), trajectory(:, 2));
    % % 如果存在这样的点，则在该点位置绘制一个洋红色的点
    if ~isempty(threshold_index2)
        plot(trajectory.globaltime(threshold_index2), trajectory.y(threshold_index2), 'm*', 'MarkerSize', 5);
    end
    % 在当前图形窗口中为当前轨迹绘制散点图
    % 保持当前图形，以便在同一个图上绘制多个轨迹
    % plot(trajectory.globaltime, trajectory.y, 'g-');
    hold on
...
% 定义需要标红的特殊轨迹索引
highlight_indices =  [1,25,50,80,100,130,160,188,218,259,340];


% for i =1:size(all_fused_trajectories2,2)
%     trajectory = all_fused_trajectories2{i};
%     time = trajectory(:,1);
%         distance = trajectory(:,2);
%         plot(time,distance);
%         hold on
% end
% %%
...
% % 预先计算所有轨迹的速度范围，确保颜色映射一致
% all_speeds = [];
% for i = 1:length(trajectories)
%     if ~isempty(trajectories{i})
%         all_speeds = [all_speeds; trajectories{i}(:,3)];
%     end
% end
...
% 颜色定义
highlight_color = [1, 0, 0];   % 红色 (RGB)
normal_color = [0, 0, 0]; % 浅灰色
alpha = 0.5; % 透明度
% 遍历所有轨迹
for i = 1:size(trajectory_cell1_ngsim,1)
    trajectory = trajectory_cell1_ngsim{i};
    ...
    %     % 绘制高亮轨迹（红色）
        plot(trajectory.globaltime, trajectory.y, ...
            'Color', 'r', ...
            'LineWidth', 1.5); % 可加粗显示
    else
        % 绘制普通轨迹（带透明度的灰色）
        h = plot(trajectory.globaltime, trajectory.y);
        set(h, 'Color', [normal_color, alpha]); % 设置RGBA颜色
    end
    hold on
end
xlabel('时间');
